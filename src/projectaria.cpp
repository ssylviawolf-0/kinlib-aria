// This file is combining example.cpp and buildWall.cpp in order to get Project Aria to work 
// So we can get the screw trajectory
// And motion plan
// But 
...
M loadCSV (const std::string & path) {
    ...
    M matrix_data = Eigen::Map<const Eigen::Matrix<typename M::Scalar, M::RowsAtCompileTime, M::ColsAtCompileTime, Eigen::RowMajor>>(
        values.data(), 
        rows, 
        cols
    );
...
void saveGripperColRaw(const std::string& path,
                       const std::vector<int>& grips) {
    std::ofstream ofs(path, std::ios::trunc);
...
void makeDummyGripperCSV(const std::string& newfilename, const std::string& filename) {
    ...
    while(std::getline(file, line)) {
        ...
        bool isOnlyWhitespace = std::all_of(line.begin(), line.end(), [](unsigned char c) {
...
std::vector<double> readGripperCondition(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        // 3. Skip meta labels
        if (line.find(":") != std::string::npos || line.find("gst") != std::string::npos) {
            continue; 
        }
        try {
...
void makeSimulationGripperCSV(const std::string& newfilename, const std::string& filename, const int pre_joint, const int drop_joint) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error: Could not open file " << filename << std::endl;
        return;
...
//   makeDummyGripperCSV(std::string(KINLIB_RESOURCES_DIR) + "Demonstrations/aria_project/dummy_gripper.csv", std::string(KINLIB_RESOURCES_DIR) + "Demonstrations/aria_project/ee_traj.csv");
...
// makeDummyGripperCSV(std::string(KINLIB_RESOURCES_DIR) + "Demonstrations/aria_project/dummy_gripper.csv", std::string(KINLIB_RESOURCES_DIR) + "Demonstrations/aria_project/world_object_formatted_move_camr.csv");
  ...
  std::vector<double> gripper_condition = readGripperCondition(std::string(KINLIB_RESOURCES_DIR) + "Demonstrations/aria_project/dummy_gripper.csv");
  ...
  for(int i = 0; i < 7; i++)
  {
    if (i >= joint_axes.cols() || i >= joint_q.cols()) {
        std::cerr << "CRITICAL: Loop index i=" << i 
                  << " exceeds available matrix columns (" << joint_axes.cols() << ")!" << std::endl;
        break;
    }
    ...
    // std::cout << "joint_axes size:  " << joint_axes.rows() << "x" << joint_axes.cols() << std::endl;
    // std::cout << "joint_q size:     " << joint_q.rows()    << "x" << joint_q.cols()    << std::endl;
    // std::cout << "joint_limits size:" << joint_limits.rows() << "x" << joint_limits.cols() << std::endl;
    // std::cout << "----------------------------------------" << std::endl;
...
//   new_task_instance.object_poses[0](1,3) -= -0.1;
  ...
  makeDummyGripperCSV("new_gripper_back_to_start.csv", "guiding_poses_back_to_start.csv");
    ...
    for (const auto& goal_ee_g : new_motion_plan)
  {
    ...
    // kinlib::ErrorCodes plan_res = kin_solver.getMotionPlanWithNSP(panda,init_jnt_val, init_ee_g, goal_ee_g, motion_plan_result, plan_info,outer_threshold, inner_threshold);
    ...
    // }
    // else {
    //     // std::cout << "skipping pos " << steps << " which is: " << goal_ee_g << std::endl;
    //     break;
    // }
  ...
  makeSimulationGripperCSV("simulation_gripper.csv", "project_aria.csv", pre_gripper_joints - 1, end_block_joints - 1);
